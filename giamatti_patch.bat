...
rem purge old S3D backups
if exist "*.s3d.old" ( 
echo Deleting previous S3D backups..
del "*.s3d.old"
)
...
rem fix black splotches for Druushk/Cekenar/Lady Nevedaria wings
...
rem fix evil eye textures in various zones
...
rem -- %~2[In ]: transpfx args
   ...
   rem test to see if a '.old' version of this file exists, suggesting that the current version has previously been patched
   if not exist "%~1.old" (
